...
ToDosRequestHandler.get("/to-dos", async (request, response) => {
  try {
    ...
    response.status(500).send({
      error: `Something went wrong when trying to get the to do`,
      errorInfo: error.message,
    });
  }
});
...
const deletedTodo = await dbHandler.run(
  ...
  todoId
    ...
    response.send({todosRemove: { ...deletedTodo } });
  } catch (error) {
    response.status(500).send({
...
ToDosRequestHandler.patch("/to-dos/:id", async (request, response) => {
  ...
  todoId
    ...
    response.send({todosUpdated: { ...todoToUpdate, title, description, is_done } });
  } catch (error) {
    response.status(500).send({
